#!/usr/bin/env node
/**
 * BitLoot Bulk Kinguin PLAYSTATION GAMES Import Script
 *
 * Imports PlayStation games and accounts from a CSV file by searching Kinguin and importing matches.
 * Auto-reprices based on your pricing rules after import.
 *
 * Usage:
 *   node scripts/bulk-import-playstation.js
 *
 * Prerequisites:
...
const CSV_FILE = path.join(__dirname, '../docs/Pre-Launch/kinguin_playstation_products.csv');
...
let ADMIN_TOKEN = process.env.ADMIN_TOKEN || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI3NWI1ZTVkOC05N2E5LTRhNGUtYjFjNC1jYTcxOGVlMjRhNzciLCJlbWFpbCI6ImJpdGxvb3QuYml6QGdtYWlsLmNvbSIsImVtYWlsQ29uZmlybWVkIjp0cnVlLCJyb2xlIjoiYWRtaW4iLCJpYXQiOjE3Njk4OTI2NzUsImV4cCI6MTc2OTg5MzU3NX0.j91KVdWFyrFA4vLb15ZNWb0OY0kqZYm-c99EY7hES38';
...
async function importProduct(productId, businessCategory = 'games') {
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  BitLoot Bulk Kinguin PLAYSTATION GAMES Import');
  console.log('═══════════════════════════════════════════════════════════════');
  console.log();
  console.log('📦 Category: GAMES (PlayStation)');
  console.log();
  ...
  if (!ADMIN_TOKEN || ADMIN_TOKEN === 'YOUR_ADMIN_TOKEN_HERE') {
    ...
    console.log('4. Edit this script and set ADMIN_TOKEN');
    console.log('   OR run: ADMIN_TOKEN="your-token-here" node scripts/bulk-import-playstation.js');
    console.log();
    process.exit(1);
  }
  ...
  console.log(`📊 Found ${products.length} PlayStation products to import`);
  console.log();
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    for (const product of batch) {
      ...
      // Import the product with businessCategory 'games'
      await sleep(DELAY_BETWEEN_IMPORTS_MS);
      const importResult = await importProduct(match.productId, 'games');
  ...
  console.log();
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  PLAYSTATION GAMES IMPORT COMPLETE');
  console.log('═══════════════════════════════════════════════════════════════');
  console.log();
  console.log(`📊 Results:`);
  ...
  if (notFoundProducts.length > 0) {
    const notFoundFile = path.join(__dirname, '../docs/Pre-Launch/playstation-import-not-found.csv');
    const notFoundCSV =
  ...
  if (importedProducts.length > 0) {
    const importedFile = path.join(__dirname, '../docs/Pre-Launch/playstation-import-success.csv');
    const importedCSV =
  ...
  console.log();
  console.log('🎉 Done! PlayStation GAMES imported.');
  console.log();
  console.log('Next steps:');
  console.log('1. Go to Admin → Catalog → Products');
  console.log('2. Filter by platform "PlayStation" to see imported products');
  console.log('3. Click "Reprice All" to apply pricing rules');
  console.log('4. Review any products in playstation-import-not-found.csv manually');
}
