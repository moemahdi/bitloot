#!/usr/bin/env node
/**
 * BitLoot Bulk Kinguin SOFTWARE Import Script
 *
 * Imports SOFTWARE products from a CSV file by searching Kinguin and importing matches.
 * Auto-reprices based on your pricing rules after import.
 *
 * Usage:
 *   node scripts/bulk-import-software.js
 *
 * Prerequisites:
...
const CSV_FILE = path.join(__dirname, '../docs/Pre-Launch/kinguin_software_products.csv');
...
let ADMIN_TOKEN = process.env.ADMIN_TOKEN || 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiI3NWI1ZTVkOC05N2E5LTRhNGUtYjFjNC1jYTcxOGVlMjRhNzciLCJlbWFpbCI6ImJpdGxvb3QuYml6QGdtYWlsLmNvbSIsImVtYWlsQ29uZmlybWVkIjp0cnVlLCJyb2xlIjoiYWRtaW4iLCJpYXQiOjE3Njk4NjE4MDUsImV4cCI6MTc2OTg2MjcwNX0.qwdqZXBWcZVn3DK3ZtrMlwm-sXN55XJXpXyl6QfBkOQ';
...
async function importProduct(productId, businessCategory = 'software') {
  // Pass businessCategory as query param to override auto-detection
  const url = `${API_BASE}/admin/catalog/kinguin/import/${productId}?businessCategory=${businessCategory}`;
...
async function updateProductCategory(productId, category) {
  const url = `${API_BASE}/admin/catalog/products/${productId}`;
  ...
  try {
    ...
    if (!response.ok) {
      const text = await response.text();
      console.error(`  ⚠️ Category update failed: ${response.status} - ${text}`);
      return false;
    }

    return true;
  } catch (error) {
    console.error(`  ⚠️ Category update error: ${error.message}`);
    return false;
  }
}
...
async function main() {
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  BitLoot Bulk Kinguin SOFTWARE Import');
  console.log('═══════════════════════════════════════════════════════════════');
  console.log();
  console.log('📦 Category: SOFTWARE');
  console.log();
  ...
  if (!ADMIN_TOKEN) {
    ...
    console.log('4. Run: ADMIN_TOKEN="your-token-here" node scripts/bulk-import-software.js');
    console.log();
    process.exit(1);
  }
  ...
  console.log(`📊 Found ${products.length} SOFTWARE products to import`);
  console.log();
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    for (const product of batch) {
      ...
      const bitlootProductId = importResult.productId;

      if (importResult.isNew === false) {
        stats.alreadyExists++;
        console.log(`  ℹ️  Already exists: ${bitlootProductId}`);
      } else {
        stats.imported++;
        console.log(`  ✅ Imported: ${bitlootProductId}`);
      }

      // Update category to 'software' for both new and existing products
      const categoryUpdated = await updateProductCategory(bitlootProductId, 'software');
      if (categoryUpdated) {
        stats.categoryUpdated++;
        console.log(`  📁 Category set to: software`);
      }

      importedProducts.push({
        title: product.title,
        kinguinId: match.productId,
        bitlootId: bitlootProductId,
        price: match.price,
  ...
  console.log();
  console.log('═══════════════════════════════════════════════════════════════');
  console.log('  SOFTWARE IMPORT COMPLETE');
  console.log('═══════════════════════════════════════════════════════════════');
  console.log();
  console.log(`📊 Results:`);
  ...
  if (notFoundProducts.length > 0) {
    const notFoundFile = path.join(__dirname, '../docs/Pre-Launch/software-import-not-found.csv');
    const notFoundCSV =
  ...
  if (importedProducts.length > 0) {
    const importedFile = path.join(__dirname, '../docs/Pre-Launch/software-import-success.csv');
    const importedCSV =
      'title,kinguin_id,bitloot_id,price,is_new\n' +
      importedProducts.map((p) => `"${p.title.replace(/"/g, '""')}",${p.kinguinId},${p.bitlootId},${p.price},${p.isNew}`).join('\n');
    fs.writeFileSync(importedFile, importedCSV);
    console.log(`📝 Imported products saved to: ${importedFile}`);
  }

  console.log();
  console.log('🎉 Done! SOFTWARE products imported and categorized.');
  console.log();
  console.log('Next steps:');
  console.log('1. Go to Admin → Catalog → Products');
  console.log('2. Filter by category "software" to see imported products');
  console.log('3. Click "Reprice All" to apply pricing rules');
  console.log('4. Review any products in software-import-not-found.csv manually');
}
